	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			if(ventana.getOperando1().getText().equals("")) {
				JOptionPane.showMessageDialog(ventana,"La primera casilla está vacía. Introduce algún número", "¡¡ESPABILA!!", JOptionPane.ERROR_MESSAGE);
			}
			else {
				...
				if(e.getSource()==ventana.getBotonSuma()) {
					...
					if(ventana.getOperando2().getText().equals("")) {
						JOptionPane.showMessageDialog(ventana,"La segunda casilla está vacía. Introduce algún número para realizar esta operación", "¡¡ESPABILA!!", JOptionPane.ERROR_MESSAGE);	
					}
					else {
					...
					if(ventana.getOperando2().getText().equals("")) {
						...
						b = Double.parseDouble(ventana.getOperando2().getText());
						if(b==0) {
							JOptionPane.showMessageDialog(ventana,"No se puede dividir por 0", "Hay que repasar esas mates...", JOptionPane.ERROR_MESSAGE);
						}
						else {
							ventana.setResultado(a/b);
						}
				...
				//Mostramos la ventana de informacion
					...
					JOptionPane.showMessageDialog(ventana,"Funcionalidad NO disponible :(", "Raiz Cuadrada", JOptionPane.INFORMATION_MESSAGE);
				}
				//Pedimos la contraseña
				else if(e.getSource()==ventana.getBotonRaiz3()){
					String password = JOptionPane.showInputDialog(null,"Contraseña","Introduzca la clave",JOptionPane.QUESTION_MESSAGE);
					if(password.equals(PASS)) {
						b = 1D/3D; //Especificamos que sean double, si no emplearia int
						ventana.setResultado(Math.pow(a,b));
					}
					else {
						JOptionPane.showMessageDialog(ventana,"No has dicho la palabra mágica", "AH, AH, AH!", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
			...
			JOptionPane.showMessageDialog(ventana,"Los valores introducidos deben ser números", "¡¡¡ANIMAL!!!", JOptionPane.ERROR_MESSAGE);
		}finally {
